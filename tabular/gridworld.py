...
class GridWorld:
    ...
    def draw_rewards(self):
        ...
        for row in range(self.height):
            for col in range(self.width):
                ...
                FONT = pg.font.SysFont('monospace',
                                       font_size)  # SysFont creates a font object from available pygame fonts
                color = RED
                if (self.mdp.rewards[state] == -0.1):
                    color = RED
                elif (self.mdp.rewards[state] > -1):
                    color = BLUE
                str_reward = str(decimal.Decimal(str(self.mdp.rewards[state])).quantize(decimal.Decimal(10) ** -2))
                SURFACEFONT = FONT.render('' + str_reward, True, color,
                                          WHITE)  # True is for anti-aliasing, looks better when true
                if (self.goal == (row, col)):
                    SURFACEFONT = FONT.render('' + str_reward, True, color,
                                              GOALCOLOR)  # True is for anti-aliasing, looks better when true
    ...
    def play(self, policy):
        ...
        while not done:
            ...
            # add new state to demonstration
            delay = 100
            if state == self.goal or len(demo) > 30:
                delay = 1000
                done = True
